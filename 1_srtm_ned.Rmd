...
getwd()
setwd("C:/Users/John/Documents/WR674/mining_impacts_to_elevation/")
getwd()
...
### Minimum change is -73 m and max chane is 38.4 m 
```
...
###reprohected buffered az_mine dem to match the dem_diff projection
az_mine_buff_reproj <- az_mine_buff %>%
  st_transform(4326)
...
####mask the dem to the area outside the mine
dem_diff_nomine <- mask(dem_diff, az_mine_buff_reproj, inverse = TRUE)

plot(dem_diff_nomine)
...
###looka at differences in elevation areas
summary(dem_diff_buff)
plot(dem_diff_buff)

###Max diff: 33.2
###Min diff: 5-51.8
####Median -1.12 
###Suggests that there is ~1 m  of difference between the two DEMs with some autliers in certain areas
...
##Reproject mine dataset to match dem_diff dataset
az_mine_reproj <- az_mine %>%
  st_transform(4326)
#mapview(dem_diff)

###Mask dem to area within the mine
dem_diff_mine <- mask(dem_diff, az_mine_reproj, inverse = FALSE)
plot(dem_diff_mine)
...
###Extract elevation areas

unmined_dem <- getValues(dem_diff_nomine)
...
###PLot differences in DEMs for both the mined and unmined areas                      
mined_table %>%
  ggplot() +
  geom_density(aes(x = dem_values, linetype = type), size = 1) +
  theme_few() +
  scale_linetype_manual(values = c("solid", "dotted"))
  

###The error distribution within the unmined area looks normal, i.e. difference in DEMs are normally distributed in the unmined region. DEM difference is right skewed in the mined region (which makes sense, because of the mining)
